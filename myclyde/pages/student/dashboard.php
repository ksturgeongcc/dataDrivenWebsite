...
// this provides a database connection
$student = $conn->prepare("SELECT

-- update this query to show all course and unit information for the student
    s.firstname,
    s.surname,
    s.email,
    c.name,
...
$student->bind_result($firstname, $surname, $email, $course_name, $award, $term );
$student->fetch();
  ...
  <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
    <div class="relative overflow-hidden rounded-lg bg-gray-600 px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
      <dt>
        ...
        <p class="ml-16 truncate text-sm font-medium text-gray-300">Student Information</p>
      </dt>
      <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
        <p class="text-2xl font-semibold text-gray-100"><?= $firstname . ' ' . $surname ?></p>
        <p class="ml-16 truncate text-sm font-medium text-gray-300"><?= $email ?></p>
    ...
    <div class="relative overflow-hidden rounded-lg bg-gray-600 px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6">
      <dt>
        ...
        <p class="ml-16 truncate text-sm font-medium text-gray-300">Course Information</p>
      </dt>
      <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
        <p class="text-2xl font-semibold text-gray-100"><?= $course_name ?></p>
        <p class="ml-2 flex items-baseline text-sm font-semibold text-green-600">
          <svg class="h-5 w-5 flex-shrink-0 self-center text-green-500" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fill-rule="evenodd" d="M10 17a.75.75 0 01-.75-.75V5.612L5.29 9.77a.75.75 0 01-1.08-1.04l5.25-5.5a.75.75 0 011.08 0l5.25 5.5a.75.75 0 11-1.08 1.04l-3.96-4.158V16.25A.75.75 0 0110 17z" clip-rule="evenodd" />
          </svg>
